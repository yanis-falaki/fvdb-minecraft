...
void serializeWorldChunksInSeries(NBTParser::GlobalPalette& globalPalette, std::string worldName, int32_t minimumSectionY);
...
int main() {
    ...
    for (auto const& world : std::filesystem::directory_iterator{worlds}) {
        serializeWorldChunksInSeries(globalPalette, world.path().filename(), 0); // minimumSectionY is zero. (World also exists in negative coodinates but I'm only interested in the surface)
    }
}

// TODO refactor and make multithreaded
void serializeWorldChunksInSeries(NBTParser::GlobalPalette& globalPalette, std::string worldName, int32_t minimumSectionY) {
